...
async function getS3URL({ fileName, basePath = 'images' }) {
  ...
  try {
    return await getSignedUrl(s3, new GetObjectCommand(params), { expiresIn: 86400 }); // URL expires in 24 hours
  } catch (error) {
...
function extractS3FilePath(url) {
  ...
  return decodeURIComponent(urlObj.pathname.substring(1)); // Remove leading "/" and decode it
}
...
const deleteS3FileByPath = async (req, file) => {
  ...
  try {
    await deleteFiles(file.filepath);
...
async function uploadFileToS3({ req, file, file_id }) {
  ...
  try {
    await s3.send(new PutObjectCommand(params));
    await fs.promises.unlink(inputFilePath); // Delete local file
    return { filepath: await getS3URL({ fileName, basePath: userId }), bytes };
  } catch (error) {
    logger.error('Error uploading file to S3:', error.message);
    throw error;
  }
...
async function getS3FileStream(filePath) {
  ...
  try {
    ...
    return Body; // AWS SDK v3 returns a stream in Body
  } catch (error) {
